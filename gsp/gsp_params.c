...
#define TUNING_DEFAULTS \
     ...
     * count ~ V x 4096/42 ~ V x 97.5). OV ~ 28 V (above 6S full-charge 25.2 V,
...
static const GSP_PARAMS_T profileDefaults[7] = {
    ...
    [GSP_PROFILE_5010] = {
        ...
        /* AN1078 SMC tuning — U3 KV700 GarudaESE bench (2026-07-13).
         * ThetaBase compensates the observer's BEMF-filter phase lag so
         * the drive angle Theta tracks the true rotor angle:
         *     Theta = pll.theta_est - 90deg + (BASE + K*|omega|)
         * and at lock pll.theta_est lags the true BEMF vector by the
         * two-stage Kslf LPF phase phi_filt, so we need BASE+K*|omega| = phi_filt.
         *
         * DERIVATION of the 200 -> 3450 change:
         *   The OLD BASE=200 (20deg) was co-calibrated with the wrong-frame
         *   dead-time comp (AN_DT_COMP_FRAC=-0.20) that we ZEROED this session.
         *   With that poison removed, the naked observer showed a CONSTANT
         *   residual angle error st = -35deg at the CL operating point
         *   (constant because Kslf = omega*Ts tracks the signal freq, so the
         *   LPF lag is ~speed-independent -> lives in BASE, not K).
         *   Null it: BASE = 20deg - 35deg = -15deg = 345deg -> degx10 = 3450.
         *   Bench-confirmed: st ~ 0 through the OL ramp, clean OL->CL handoff,
         *   observer-driven CL holds the speed setpoint + rejects a hand-load
         *   (only the 8A fast-OC trips it, not angle loss). See memory
         *   akesc_garudaese_an1078_ol_cl_handoff_2026_07_13.
         * K × 1e7 = 800 → 8.0e-5 rad/(rad/s elec) (adds +15deg @ 32k eRPM;
         *   left as-tuned. If the lag is truly constant K should trend to 0 —
         *   revisit if angle drifts at very different speeds).
