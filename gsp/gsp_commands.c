...
#endif
/* GarudaESE board layer — engine-independent (needed in 6-step builds too) */
#include "hal/hal_ata6847.h"     /* per-start GDU re-arm + ATA diag */
#include "hal/board_service.h"   /* g_ataReady */
...
static uint32_t BuildFeatureFlags(void)
{
    ...
    if (FEATURE_LIVE_TUNE)           f |= (1UL << 25);  /* safe 6-step knobs settable while spinning */
    return f;
}
...
static void HandleGetInfo(const uint8_t *payload, uint8_t payloadLen)
{
    ...
    info.boardId         = GSP_BOARD_GARUDAESE;   /* was MCLV — host keys display scaling on this */
...
static void HandleStartMotor(const uint8_t *payload, uint8_t payloadLen)
{
    ...
    /* GarudaESE: the ATA6847 latches its outputs OFF on internal protection
     * trips (SCP/VDS/ILIM escalation) — a boot-time GDU Normal entry does not
     * survive them, which made only the FIRST run after power-up conduct.
     * Mirror the reference firmware: re-arm the gate driver on EVERY start
     * (capture+clear latched faults, Standby->Normal, verified poll). Runs in
     * main-loop context (GSP_Service), so the ~10 ms verify poll is safe. */
    HAL_ATA6847_ClearFaults();
    g_ataReady = HAL_ATA6847_EnterGduNormal();
    if (!g_ataReady) {
        SendError(GSP_ERR_BUSY);   /* gate driver refused Normal — see ATA_DIAG */
        return;
    }
...
/* GarudaESE gate-driver diagnostics: raw ATA6847 registers + the last GDU
 * bring-up attempt result. Reading SIR1..SIR5 CLEARS latched fault flags in
 * the device, so capture the values BEFORE clearing faults elsewhere. */
static void HandleAtaDiag(const uint8_t *payload, uint8_t payloadLen)
{
    (void)payload;
    (void)payloadLen;

    uint8_t d[13];
    HAL_ATA6847_ReadDiag(d);            /* [DSR1 DSR2 SIR1..SIR5 GOPMCR] */
    d[8]  = gAta_LastDsr1AtNormal;
    d[9]  = (uint8_t)(gAta_LastGduAttempts & 0xFFu);
    d[10] = (uint8_t)(gAta_LastGduAttempts >> 8);
    d[11] = gAta_LastGduResult;
    d[12] = g_ataReady ? 1u : 0u;
    GSP_SendResponse(GSP_CMD_ATA_DIAG, d, sizeof(d));
}
...
static void HandleSetParam(const uint8_t *payload, uint8_t payloadLen)
{
    ...
    /* FEATURE_LIVE_TUNE (GarudaESE port): a SAFE subset of 6-step knobs that
     * the control loop already reads live in the fast ISR (timing advance, ZC
     * blanking/deadband/demag, sync/filter thresholds, duty slew) may be set
     * while spinning so the GUI sliders take effect in real time.  Protection
     * thresholds (Vbus/OC) and startup params are deliberately NOT included --
     * they still require IDLE to avoid a mid-run discontinuity or a safety
     * change under load. */
    bool is_live_tunable = false;
#if FEATURE_LIVE_TUNE
    switch (paramId) {
        case PARAM_ID_TIMING_ADV_MAX_DEG:
        case PARAM_ID_ZC_BLANKING_PCT:
        case PARAM_ID_ZC_ADC_DEADBAND:
        case PARAM_ID_ZC_DEMAG_DUTY_THRESH:
        case PARAM_ID_ZC_DEMAG_BLANK_EXTRA:
        case PARAM_ID_ZC_SYNC_THRESHOLD:
        case PARAM_ID_ZC_FILTER_THRESHOLD:
        case PARAM_ID_DUTY_SLEW_UP:
        case PARAM_ID_DUTY_SLEW_DOWN:
            is_live_tunable = true;
            break;
        default:
            break;
    }
    ...
    if (!is_an1078_live && !is_oc_live && !is_live_tunable && garudaData.state != ESC_IDLE) {
        SendError(GSP_ERR_WRONG_STATE);
        return;
    }
...
static const CMD_ENTRY_T cmdTable[] = {
    ...
    { GSP_CMD_ATA_DIAG,        0, HandleAtaDiag        },
    /* Phase 1: params */
