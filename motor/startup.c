...
/* FOC builds (2026-07-09, ported from flagship dspic33AKESC): the 6-step sine-start
 * constants (SINE_ALIGN_ANGLE_Q16, SINE_ERPM_TO_ANGLE_Q16, SINE_CENTER_DUTY,
 * SINE_PHASE_OFFSET_Q16) live inside garuda_calc_params.h's !FOC dependency-guard
 * region, so they are undefined when a FOC engine owns the loop. Compile the whole
 * sine machinery out here — the FOC engines run their own startup. Local to this TU. */
...
#else
        /* GarudaESE: no bus-current sense in BEMF mode -> no ramp current gate */
        if (1)
#endif
        {
