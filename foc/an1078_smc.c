...
void AN_SMC_Position_Estimation(AN_SMC_T *s)
{
    ...
    /* OmegaFltred = LPF of the PLL speed (2026-07-11 MCLV-port). Feeding
     * s->pll.omega_est RAW per-tick into the speed PI let the +/-2000 eRPM
     * bounce wind the PI to an OC current spike at the OL->CL handoff; the LPF
     * is what makes observer-angle drive stable on MCLV (stock AN1078 feeds a
     * boxcar+LPF-smoothed Omega). Used by Kslf scaling and (in motor.c) by the
     * speed PI when CL is active. */
    s->OmegaFltred += AN_OMEGA_FILT_COEF
                    * (s->pll.omega_est - s->OmegaFltred);
