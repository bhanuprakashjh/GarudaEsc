...
/* GarudaESE: ibusRaw/ibusAvg/ibusMax un-gated from FEATURE_HW_OVERCURRENT --
 * the ISR owns the bus-current window on this board (ATA op-amp path; note
 * the CSA is inactive while the ATA is in BEMF-comparator mode, so this may
 * read the rest bias until a non-BEMF config is used). The dsPIC OA3/CMP3
 * trip machinery stays gated below. */
#if 1  /* was: FEATURE_HW_OVERCURRENT */
