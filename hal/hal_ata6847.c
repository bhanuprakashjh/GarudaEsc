...
#include <xc.h>
#include "garuda_board.h"      /* GarudaESE: FCY + ILIM_DAC, before libpic30.h */
...
void HAL_ATA6847_Init(void)
{
    ...
    /* 2026-07-11 ILIM chop DISABLED (bit7=0). Tried (1<<7) + ILIM_DAC=40 as a HW
     * backstop for the handoff spike; DAC=40 chopped BELOW the 5A align current,
     * so the bridge never came up -> firmware BUSY / would not start (confirmed
     * bench, survived a full power-cycle). Bracketing: DAC=40 -> chop <5A;
     * DAC=120 -> chop ~15-25A. The window that catches the 9A handoff spike but
     * clears the 5A align is narrow (~DAC 50-65) and the DAC->amps map is
     * unknown, so ILIM-chop tuning is deferred. Back to known-good (starts, ramps
     * clean no-prop, OCs at handoff). REVERT to (1<<7) only with a bracketed DAC. */
