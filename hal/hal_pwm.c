...
void InitPWMGenerator1(void)
{
    ...
    PG1STAT     = 0x0000;
    PG1IOCON1 = 0x0000; PG1IOCON2 = 0x0000;
...
#ifdef ENABLE_PWM_FAULT_PCI
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCI     = 0x0000;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.TSYNCDIS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.TERM = 1;           /* Auto-terminate when qualifier goes false */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.AQPS = 1;           /* Inverted: accept when LEB LOW (post-blanking) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.AQSS = 0b010;       /* LEB active as acceptance qualifier */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.PSYNC = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.PPS = 1;            /* Inverted polarity */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.PSS = 0b01000;      /* RPn input (PCI8R = board U25B) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.BPEN = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.BPSEL = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.TERMPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.ACP = 3;            /* Latched */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.LATMOD = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.TQPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.TQSS = 0;
#if FEATURE_HW_OVERCURRENT && OC_PROTECT_MODE == 1
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.PSS = 0b11101;      /* CMP3 (replaces RPn/PCI8R) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.PPS = 0;            /* Non-inverted */
#endif
#else
    ...
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCI     = 0x0000;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.ACP  = 1;       /* Edge-sensitive (requires qualifier) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.AQSS = 0b010;   /* LEB active as acceptance qualifier */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.AQPS = 0;       /* Non-inverted: need LEB HIGH to accept */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG1FPCIbits.TERM = 1;       /* Safety: auto-terminate if somehow accepted */
#endif

#if FEATURE_HW_OVERCURRENT && (OC_PROTECT_MODE == 0 || OC_PROTECT_MODE == 2) && OC_CLPCI_ENABLE
    PG1CLPCI1 = 0x0000; PG1CLPCI2 = 0x0000;
    ...
    PG1CLPCI1 = 0x0000; PG1CLPCI2 = 0x0000;
#endif
    PG1FFPCI1 = 0x0000; PG1FFPCI2 = 0x0000;
    PG1SPCI1 = 0x0000; PG1SPCI2 = 0x0000;
    ...
    PG1DCA      = 0x0000;
    PG1PER      = 0x0000;
    /* 2026-07-10 SHOOT-THROUGH EXPERIMENT: MCU dead-time ZEROED on all PGs.
     * Evidence: every Run press = PSU Vbus sag (real current with zero
     * commanded volts) -> ADC wedges ~15 ms in -> BOR+EXTR reset. Our combo
     * (complementary PMOD=0 + center-aligned + POLH=1 inverted HS + MCU DT)
     * is unproven on this silicon; if DT is inserted on the post-polarity
     * pin edges, the inverted HS turns 300 ns of dead-time into 300 ns of
     * OVERLAP = shoot-through every edge. The ATA6847's internal ADAPTIVE
     * dead-time (GDUCR3 ADDTHS/ADDTLS=50-160 ns, VDS-monitored) remains the
     * shoot-through protection — same philosophy as the bench-proven
     * reference FW (independent mode + ATA internal DT). Restore
     * DEADTIME_COUNTS if this proves unrelated. */
    PG1DTbits.DTH = 0;   /* 2026-07-10: was DEADTIME_COUNTS — see note above PG1DT */
    PG1DTbits.DTL = 0;
...
void InitPWMGenerator2(void)
{
    ...
    PG2STAT     = 0x0000;
    PG2IOCON1 = 0x0000; PG2IOCON2 = 0x0000;
    ...
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCI     = 0x0000;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.TSYNCDIS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.TERM = 1;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.AQPS = 1;           /* Inverted: accept when LEB LOW */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.AQSS = 0b010;       /* LEB as acceptance qualifier */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.PSYNC = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.PPS = 1;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.PSS = 0b01000;      /* RPn input (PCI8R = board U25B) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.BPEN = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.BPSEL = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.TERMPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.ACP = 3;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.LATMOD = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.TQPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.TQSS = 0;
#if FEATURE_HW_OVERCURRENT && OC_PROTECT_MODE == 1
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.PSS = 0b11101;      /* CMP3 (replaces RPn/PCI8R) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.PPS = 0;            /* Non-inverted */
#endif
#else
    /* FOC: FPCI disabled via LEB qualifier gating (see PG1 comment) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCI     = 0x0000;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.ACP  = 1;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.AQSS = 0b010;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.AQPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG2FPCIbits.TERM = 1;
#endif

#if FEATURE_HW_OVERCURRENT && (OC_PROTECT_MODE == 0 || OC_PROTECT_MODE == 2) && OC_CLPCI_ENABLE
    PG2CLPCI1 = 0x0000; PG2CLPCI2 = 0x0000;
    ...
    PG2CLPCI1 = 0x0000; PG2CLPCI2 = 0x0000;
#endif
    PG2FFPCI1 = 0x0000; PG2FFPCI2 = 0x0000;
    PG2SPCI1 = 0x0000; PG2SPCI2 = 0x0000;
    ...
    PG2DCA      = 0x0000;
    PG2PER      = 0x0000;
    PG2DTbits.DTH = 0;   /* 2026-07-10: was DEADTIME_COUNTS — see note above PG1DT */
    PG2DTbits.DTL = 0;
...
void InitPWMGenerator3(void)
{
    ...
    PG3STAT     = 0x0000;
    PG3IOCON1 = 0x0000; PG3IOCON2 = 0x0000;
...
#ifdef ENABLE_PWM_FAULT_PCI
    /* LEB-gated FPCI acceptance — see PG1 comment */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCI     = 0x0000;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.TSYNCDIS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.TERM = 1;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.AQPS = 1;           /* Inverted: accept when LEB LOW */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.AQSS = 0b010;       /* LEB as acceptance qualifier */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.PSYNC = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.PPS = 1;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.PSS = 0b01000;      /* RPn input (PCI8R = board U25B) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.BPEN = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.BPSEL = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.TERMPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.ACP = 3;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.LATMOD = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.TQPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.TQSS = 0;
#if FEATURE_HW_OVERCURRENT && OC_PROTECT_MODE == 1
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.PSS = 0b11101;      /* CMP3 (replaces RPn/PCI8R) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.PPS = 0;            /* Non-inverted */
#endif
#else
    /* FOC: FPCI disabled via LEB qualifier gating (see PG1 comment) */
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCI     = 0x0000;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.ACP  = 1;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.AQSS = 0b010;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.AQPS = 0;
    // [GarudaESE] FPCI removed (no Fault-PCI on AK256MC506): PG3FPCIbits.TERM = 1;
#endif

#if FEATURE_HW_OVERCURRENT && (OC_PROTECT_MODE == 0 || OC_PROTECT_MODE == 2) && OC_CLPCI_ENABLE
    PG3CLPCI1 = 0x0000; PG3CLPCI2 = 0x0000;
    ...
    PG3CLPCI1 = 0x0000; PG3CLPCI2 = 0x0000;
#endif
    PG3FFPCI1 = 0x0000; PG3FFPCI2 = 0x0000;
    PG3SPCI1 = 0x0000; PG3SPCI2 = 0x0000;
    /* LEB counter — see PG1 comment. Required for FPCI qualifier. */
#if FEATURE_HW_OVERCURRENT
    PG3LEB      = 0x0000;
    PG3LEBbits.PHR = 1;
    ...
    PG3DCA      = 0x0000;
    PG3PER      = 0x0000;
    PG3DTbits.DTH = 0;   /* 2026-07-10: was DEADTIME_COUNTS — see note above PG1DT */
    PG3DTbits.DTL = 0;
