...
/* FOC builds (2026-07-09, ported from flagship dspic33AKESC): the 6-step BEMF-ZC
 * sampling glue in this TU reads phaseB_val/phaseAC_val, which the FOC ADC mapping
 * replaces with ia/ib (AD1CH0=Ia, AD2CH0=Ib). FEATURE_BEMF_CLOSED_LOOP stays =1 in
 * garuda_config.h for the shared 6-step scaffolding, so compile the 6-step-only ISR
 * blocks OUT of this file when any FOC engine owns the loop — otherwise phaseB_val/
 * phaseAC_val are undeclared. Local to this translation unit only. */
...
/* BRING-UP 2026-07-10: ADC-ISR liveness counter — the bench shows the whole
 * FOC fast path freezing ~16 ms into ALIGN while main/Timer1 stay alive.
 * Snapshot exports this + live trigger registers so we can see WHERE it died.
...
void __attribute__((__interrupt__, no_auto_psv)) GARUDA_ADC_INTERRUPT(void)
{
     ...
     * simultaneously each PG1TRIGA). Phase A=U, B=V, C=W. */
    (void)(uint16_t)ADCBUF_IA;              /* AD1CH0 = Iu (anchor read) */
    uint16_t phaseU_val = ADCBUF_BEMF_U;
    uint16_t phaseV_val = ADCBUF_BEMF_V;
    uint16_t phaseW_val = ADCBUF_BEMF_W;
    /* Compat aliases for legacy diagnostic / coast-listen / I-f paths that
     * still name the MCLV channels. Phase B = V; the old "A/C muxed" value
     * maps to U here (dedicated channels make A/C explicit elsewhere). */
    uint16_t phaseB_val = phaseV_val;
    uint16_t phaseAC_val = phaseU_val;
    (void)phaseB_val; (void)phaseAC_val;
#endif
    garudaData.vbusRaw = ADCBUF_VBUS;
    garudaData.potRaw = ADCBUF_POT;
    g_adcIsrCount++;   /* BRING-UP: ISR liveness probe (read by snapshot) */
    ...
    /* Store floating phase ADC value in bemfRaw with validity tracking */
    {
        ...
        /* GarudaESE: each phase has its OWN dedicated ADC channel — sampled
         * simultaneously, so the value is always valid (no mux settle). */
        if (fp == FLOATING_PHASE_A)
            garudaData.bemf.bemfRaw = phaseU_val;
        else if (fp == FLOATING_PHASE_B)
            garudaData.bemf.bemfRaw = phaseV_val;
        else /* FLOATING_PHASE_C */
            garudaData.bemf.bemfRaw = phaseW_val;
        garudaData.bemf.bemfSampleValid = true;
         ...
         */
        {
            ...
            ss.obs_x2   = 0;
            {
                ...
                uint32_t erpm_hr = (garudaData.hwzc.enabled && garudaData.hwzc.stepPeriodHR)
                                 ...
                                 : 0;
#else
                uint32_t erpm_hr = 0;   /* GarudaESE: hwzc engine off (software ZC) */
#endif
            ...
            ss.flags  =
#if FEATURE_ADC_CMP_ZC
                        (garudaData.hwzc.enabled ? 0x01 : 0x00) |
#endif
...
#else
#if !FEATURE_FOC && !FEATURE_FOC_V2 && !FEATURE_FOC_V3 && !FEATURE_FOC_AN1078
    garudaData.bemf.bemfRaw = phaseV_val;   /* OL: any floating-phase sample */
    (void)phaseU_val; (void)phaseW_val;
#endif
#endif
...
#if FEATURE_VBUS_FAULT
    {
            ...
            || garudaData.state == ESC_IF_RAMP)   /* I-f spin-up: keep OV/UV active */
        {
             ...
             * (2026-07-09, ported from flagship). vbusRaw is one unfiltered ADC
             * reading at the PWM-relative trigger; under FOC's 45kHz PWM it
             * carries switching-coupled dips that read >=3 consecutive samples
             * low and false-trip UV even with the DC bus steady (bench: 15.9V,
             * 10A PSU headroom, yet UV fired). A real brownout/regen evolves
             * over ms and passes the filter unattenuated, so protection stands.
             * Re-seeded each run via the else branch. */
            ...
            /* 2026-07-09 DIAGNOSTIC UNBLOCK: bypass the software UV during FOC
             * ALIGN/OL_RAMP. Bench proof it's a mis-read not a brownout: PSU
             * drops only "a little" (~15V) yet UV fires at <7.4V, and the
             * vbusFilt average still trips -> the per-cycle vbusRaw sample reads
             * systematically low under the 45kHz FOC PWM. Hardware CMP3 + board
             * FPCI + the CC-limited PSU remain the real protection here. UV stays
             * ACTIVE in closed loop and for all 6-step states. */
            bool uvCheckActive = true;
#if FEATURE_FOC_AN1078
            if (garudaData.state == ESC_ALIGN || garudaData.state == ESC_OL_RAMP)
                uvCheckActive = false;
#endif
            if (uvCheckActive && vbusFilt < uvThreshold)
            {
        ...
        else
        {
            vbusOvCount = 0;
            vbusUvCount = 0;
            vbusFiltInit = false;   /* re-seed the OV/UV filter on next run */
        }
    }
#endif
...
#if OC_PROTECT_MODE == 2
    {
...
#if FEATURE_FOC_AN1078
        /* FOC OC source (2026-07-09, ported from flagship cde35a2): the bus-
         * current ADC reads a sustained switching-ring phantom under the FOC's
         * 45 kHz PWM (forensics: ibusRaw=4012 counts / +21 A with real current
         * ~zero) → instant phantom OC_SW at ALIGN. Watch the FOC's own offset-
         * calibrated phase currents instead: worst phase |i| vs focFaultOcCentiA
         * (centiamps, GUI-live-tunable). The bus check stays as a second layer
         * (it caught a REAL runaway when the phase scale was wrong) but with the
         * corrected AN_CURRENT_A_PER_COUNT the phase path is now the primary. */
        ...
        /* 2026-07-09 BUS CLAUSE DROPPED in FOC mode: the 45 kHz switching-ring
         * phantom RAILS the bus ADC (forensic ibusRaw=4012 of 4095 full-scale
         * with real current ~zero), so `ibusRaw > OC_FAULT_ADC_VAL` trips on
         * every ALIGN tick during warmup (Vd=Vq=0, zero winding current) — this
         * was the instant OC_SW. No bus threshold can separate phantom from real
         * because the phantom saturates the ADC. Phase current (calibrated, now
         * correctly scaled) is the sole software OC layer; CMP3/board FPCI +
         * PSU current-limit are the hardware backstops. Bus check stays for
         * 6-step (24 kHz, no phantom) via the #else branch. */
        if (iMag > ocLimitA)
        {
     ...
     */
    {
        ...
        static uint32_t s_an_start_count = 0;   /* BRING-UP: restart-cycle probe */
        ...
        if (s_foc_an.mode == AN_MODE_FAULT) {
            HAL_MC1PWMDisableOutputs();     /* every tick: keep the bridge dead */
            if (garudaData.faultCode == FAULT_NONE)
                garudaData.faultCode = s_foc_an.faultCode
                                     ? s_foc_an.faultCode : FAULT_FOC_INTERNAL;
            garudaData.runCommandActive = false;
            /* DO NOT clear LED2 here.  This block runs every ADC ISR tick (45 kHz)
             * while latched in fault; a per-tick LED2=0 stomps the main-loop
             * ESC_FAULT fast-blink (main.c:262, ~10 Hz) within 22 us of every
             * toggle -> LED sits dark -> board LOOKS wedged even though the ISR
             * is alive (isr= keeps counting, telemetry streams).  The main loop
             * owns LED2 in every state; let it drive the fault blink.
             * (2026-07-13: bench-reported "heartbeat not blinking on fault".) */
        }
        ...
        /* BRING-UP 2026-07-10: repurpose focLambdaEst (shown as "st=" in the
         * host text dump) to expose the observer's NATIVE speed estimate in
         * eRPM.  smc.OmegaFltred is CLOBBERED with startupRamp during OL
         * (an1078_motor.c:810) so the eRPM field only ever shows the FORCED
         * ramp in open loop — we have never seen what the observer actually
         * estimates while the rotor genuinely spins.  pll.omega_est is the
         * un-clobbered PLL output (rad/s elec -> eRPM = *60/2pi = *9.5493).
         * Watch: during OL, does "st=" climb to the forced eRPM (observer
         * tracks -> angle trustworthy) or stall low (observer mistracks ->
         * Rs/Ls parameterization is the culprit)? */
        /* FRAME CHECK 2026-07-10 -- ANSWERED AND REVERTED.  One run with st=
         * showing angle(measured I vector) - theta_drive proved st = +90 deg
         * CONSTANT through ALIGN, the whole OL ramp, and CL (77..101, mean 90;
         * the +-12 deg wobble is exactly 90 - atan(id/iq), i.e. the physical
         * I/f rotor hunt resolved by the sense path, not noise).  VERDICT: the
         * V-vs-I vector geometry is CORRECT -- no channel swap, no polarity
         * flip, no OA1/OA2 gain skew.  Current sensing on this board is fully
         * exonerated (magnitude, frequency, and now vector).  The observer-
         * angle drive failure is therefore SMC/PLL estimator dynamics for this
         * motor -> simulation, not more bench hardware hunting.
         * st= is restored to the native observer speed readout below. */
        /* 2026-07-11 DEAD-TIME DIAGNOSIS: st= now shows the LIVE handoff angle
         * error thetaError = wrap(thetaOpenLoop - smc.Theta) in DEGREES. This is
         * exactly the quantity the OL->CL handoff gate tests (|.| < 34 deg =
         * AN_HANDOFF_ANGLE_TOL_RAD, an1078_motor.c:707). Hypothesis: the
         * uncompensated ATA6847 adaptive dead-time biases the observer angle, so
         * at NO-PROP st= parks OUTSIDE +-34 (never hands off) while under prop
         * the rotor load angle pulls it back inside. Watch st= through a no-prop
         * ramp vs a prop ramp. Revert to OA1 offset: = s_foc_an.ia_offset.
         * Pure telemetry -- observer/control path unchanged. */
        {
            float terr = s_foc_an.thetaOpenLoop - s_foc_an.smc.Theta;
            terr = fmodf(terr, 6.28318531f);            /* -> (-2pi, 2pi) */
            if (terr >  3.14159265f) terr -= 6.28318531f;
            else if (terr < -3.14159265f) terr += 6.28318531f;
            garudaData.focLambdaEst = terr * 57.2957795f;   /* st= thetaError deg */
        }
        /* BRING-UP 2026-07-10 (revert after first spin): repurpose two float
         * telemetry slots to expose the AN engine's internals — the bench
         * shows LOCK for 1.5 s with zero volts, i.e. startupLock frozen in
        ...
        garudaData.focObsGain     = s_foc_an.ib_offset;   /* production: OA2 auto-zero offset (STA GATE-2 lk=eRPM diagnostic reverted 2026-07-15) */
...

        {
            ...
            /* 2026-07-12 DIAGNOSTIC (option-2 premise check): the old clamp to
             * 1.0 HID the true over-estimate factor. conf = |E_obs| / (lambda*omega).
             * If conf ~= 1.0-1.1 in OL at 14k, the observer's voltage/BEMF estimate
             * is CORRECT -> the voltage-estimate fix is the wrong tree and the CL
             * instability is estimator dynamics (Ls/plant) or observer-drive loop
             * gain.  If conf ~= 2-3, the fed voltage really is inflated -> hunt the
             * scale.  Clamp raised to 9.99 only to keep the "conf=" field width sane
             * (host prints %4.2f).  Telemetry only -- gate uses bemf_mag directly. */
            if (conf > 9.99f) conf = 9.99f;
            garudaData.focObsConfidence = conf;
        }
        /* mode in low nibble + run/cal flags (host masks these out) */
        garudaData.focSubState = (uint8_t)s_foc_an.mode
                               | (s_foc_an.runMotor ? 0x40u : 0u)
                               | (s_foc_an.cal_done ? 0x80u : 0u);
...
#if FEATURE_BURST_SCOPE
        /* Burst scope sample from AN1078 state (2026-07-09: this branch never
         * fed the scope — 6-step/V2/V3 each had a write block, AN1078 didn't,
         * so the GUI scope tab captured nothing in FOC builds).
         * Two deliberate departures from the V3 block's scaling:
         *   theta:   int16 ×10000 holds ±3.2767 rad; theta_drive is [0,2π) —
         *            re-wrap to ±π or it overflows past 180°.
         *   obs_x:   V3 stores FLUX (×100000); AN1078's EalphaFinal/EbetaFinal
         *            are BEMF VOLTS (λω, several V at speed) → ×1000 (mV,
         *            ±32.7V range) so they don't clip at 0.33V. */
        {
            SCOPE_SAMPLE_T ss;
            ss.ia    = (int16_t)(s_foc_an.ia * 1000.0f);
            ss.ib    = (int16_t)(s_foc_an.ib * 1000.0f);
            ss.id    = (int16_t)(s_foc_an.id_meas * 1000.0f);
            ss.iq    = (int16_t)(s_foc_an.iq_meas * 1000.0f);
            ss.vd    = (int16_t)(s_foc_an.vd * 100.0f);
            ss.vq    = (int16_t)(s_foc_an.vq * 100.0f);
            {
                float th = s_foc_an.theta_drive;
                if (th > 3.14159265f) th -= 6.28318531f;   /* [0,2π) → ±π */
                ss.theta = (int16_t)(th * 10000.0f);
            }
            ss.obs_x1 = (int16_t)(s_foc_an.smc.EalphaFinal * 1000.0f);
            ss.obs_x2 = (int16_t)(s_foc_an.smc.EbetaFinal  * 1000.0f);
            {
                float omega_clamped = s_foc_an.smc.OmegaFltred;
                if (omega_clamped > 32767.0f)  omega_clamped = 32767.0f;
                if (omega_clamped < -32768.0f) omega_clamped = -32768.0f;
                ss.omega = (int16_t)(omega_clamped);
            }
            ss.mod_index = (int16_t)(garudaData.focModIndex * 10000.0f);
            ss.flags  = ((s_foc_an.mode == AN_MODE_CLOSED_LOOP) ? 0x01 : 0x00)
                      | ((garudaData.state == ESC_FAULT) ? 0x02 : 0x00)
                      | (((uint8_t)s_foc_an.mode & 0x07) << 2);
            ss.state  = (uint8_t)garudaData.state;
            ss.tick_lsb = (uint16_t)(garudaData.systemTick & 0xFFFF);
            Scope_WriteSample(&ss);
        }
...
#else
                        if (0) {   /* GarudaESE: hwzc engine off (software ZC) */
#endif
