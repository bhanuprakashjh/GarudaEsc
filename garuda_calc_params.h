...
#if GARUDA_TARGET_AK512 && !FEATURE_FOC && !FEATURE_FOC_V2 && !FEATURE_FOC_V3 && !FEATURE_FOC_AN1078
/* 6-STEP ONLY: sample at the OFF-CENTER (freewheel) point, NOT the ON-pulse center.
 * This instant is tuned for BEMF VOLTAGE sensing (floating phase clean during
 * freewheel). FOC builds MUST NOT use it — they read low-side shunt CURRENT on
 * AD1CH0/AD2CH0, which is only valid at the valley (TRIGA=0, low-side conducting).
 * Sampling current at MPER/2 gives near-zero/garbage iq_meas → the current PI
 * goes blind → vq winds to the voltage clamp → runaway (PSU fold). AN957 (this
 * board's reference) samples ALL currents at TRIGA=0. So FOC falls through to the
 * #else (ADC_SAMPLING_POINT=0) below, same as the non-AK512 path. 2026-07-09.
