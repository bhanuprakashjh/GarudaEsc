...
#define FEATURE_BEMF_CLOSED_LOOP 1  /* 2026-07-11 REVERTED to 1: V2 detect build caused PCI fault + MCU hang on bench; restored proven AN1078 6-step CL. */
...
#define FEATURE_TIMING_ADVANCE   1  /* 2026-07-11 REVERTED to 1 (was 1->0 for V2 detect build,
                                     * which broke the bench). Restored with AN1078.
                                     * (orig: Phase B3: Linear timing advance by RPM — RE-ENABLED 2026-05-26 to compensate detection-chain latency at high RPM. Original baseline schedule: 0° below 3k eRPM, linear ramp to 22° at MAX_CLOSED_LOOP_ERPM (70k for 2810), clamped 22° above. */
#define FEATURE_DYNAMIC_BLANKING 1  /* Phase C1: Speed+duty-aware blanking (extra blank at high duty/demag) */
#define FEATURE_LIVE_TUNE        1  /* Allow a SAFE subset of ISR-live 6-step knobs (timing advance, ZC
                                     * blanking/deadband/demag, sync/filter thresholds, duty slew) to be
                                     * SET via GSP while the motor spins — for live tuning from the GUI.
                                     * Protection (Vbus/OC) and startup params still require IDLE.
                                     * Advertised on featureFlags bit 25 so the GUI enables those sliders. */
...
#define FEATURE_SINE_STARTUP     1  /* 2026-07-11 REVERTED to 1 (was 1->0 for V2 detect build). Restored with AN1078. (orig: RESTORED 2026-06-10 proven sine startup (I-f parked).
...
#define FEATURE_ADC_CMP_ZC       0  /* GarudaESE: OFF for first build — use the software bemf_zc 24 kHz ZC off
                                     * the dedicated BEMF channels (AD3CH0/AD2CH1/AD5CH0). High-speed ADC-
                                     * comparator ZC + ATA6847 comparator ZC (Z1) need a channel remap —
                                     * see INTEGRATION_TODO.md. */
#define FEATURE_HW_OVERCURRENT    0  /* GarudaESE: no dsPIC OA3/CMP3 bus-current path (ATA op-amps are BEMF
                                     * comparators). OC = phase currents (OA1/OA2) + ATA SCPCR/ILIM. */
...
#define FEATURE_AM32_STARTUP    0  /* 2026-06-12 bench experiment: AM32-style "kick + listen".
...
#define ARM_BEEP_FREQ1_HZ     500  /* note 1 (first ~ARM_BEEP_MS/3) */
#define ARM_BEEP_FREQ2_HZ    800  /* note 2 */
#define ARM_BEEP_FREQ3_HZ    1000  /* note 3 — rising chirp */
#define ARM_BEEP_DUTY_PCT       4  /* drive strength — louder; = profile align duty class. 4+ NOT
...
#define FEATURE_FOC_V2           0  /* 2026-07-11 REVERTED to 0: V2 detect build caused PCI fault + MCU hang. (was 0->1 to run the V2
                                     * engine's motor AUTO-DETECT on the U3 (measures Rs, Ls, Ke
                                     * on THIS board -- our profile values are web guesses, and
                                     * Ke was computed for 700KV while the motor may be 750KV;
                                     * Ls was borrowed from a 5055). Detect test current clamped
                                     * 0.5-3A, Vclamp <= 0.3*Vbus, current sign convention matches
                                     * the proven AN_CURRENT_INVERT=1. After detect: read back
                                     * focRsMilliOhm/focLsMicroH/focKeUvSRad, then REVERT this to
                                     * 0 and FEATURE_FOC_AN1078 back to 1. */
#define FEATURE_FOC_V3           0  /* Phase J: FOC v3 — SMO observer + PLL */
#define FEATURE_FOC_AN1078       1  /* 2026-07-11 REVERTED to 1 (V2 detect build broke bench). */  /* 2026-07-10: 0->1 first FOC attempt on GarudaESE
                                     * (board gains already applied; 6-step proved telemetry).
                                     * Original note: FOC+SMO (AN1078 sliding-mode observer) on the 2810.
                                     * Flagship dspic33AKESC ran this exact engine to 223.9k eRPM.
                                     * The 6-step flags below (BEMF_CLOSED_LOOP/SINE_STARTUP/ADC_CMP_ZC)
                                     * stay =1 but become dead code — AN1078 owns the loop via its own
                                     * AN_MotorInit/Start state machine. WARNING: this is the June-24
...
#define FEATURE_ADC_POT          0  /* 2026-07-10: pot wired but DANGLING on the
                                     * bench -> floating/noisy throttle input.
                                     * GSP is the throttle source. Re-enable when
                                     * a real pot is properly connected. */
...
#define FEATURE_ARM_SWITCH       0  /* 2026-07-10: switch wire DANGLING on RD4 =
                                     * antenna; noise edges fire the edge-triggered
                                     * arm/KILL logic -> suspected engine restart-
                                     * cycling (align frozen at zero volts).
                                     * Re-enable when the switch is mounted. */
...
/* Bus Voltage */
#define VBUS_OVERVOLTAGE_ADC       2730        /* GarudaESE 13:1: ~28.6V (was 3600 = 37.7V, above the 26V TVS) */
#define VBUS_UNDERVOLTAGE_ADC      500         /* GarudaESE 13:1: ~5.2V (bench-PSU friendly; raise for 6S) */
#define VBUS_FAULT_FILTER          3           /* Consecutive ADC samples to confirm fault (3 = ~125us) */
#define VBUS_FILT_SHIFT            4           /* 2026-07-09 ported from flagship: IIR shift for the OV/UV
                                                * bus-voltage low-pass (~16 samples). Under FOC's 45kHz PWM
                                                * the single vbusRaw trigger catches switching-coupled dips
                                                * that false-trip UV with the DC bus steady (bench 15.9V). */
